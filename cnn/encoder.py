...
- CNN_Large: Large mobile-style CNN encoder (7.37M params)
...
class CNN_Large(CNNEncoderBase):
    """
    Large CNN encoder.

    An efficient mobile-style CNN architecture with 7.37M parameters,
...
_ARCH_MAP: Dict[CNNEncoderArch, type] = {
    CNNEncoderArch.large: CNN_Large,
}


def build_cnn_encoder(  # noqa: C901
    arch: Union[CNNEncoderArch, str] = CNNEncoderArch.large,
    ...
    Args:
        arch: Architecture to build (CNNEncoderArch enum or string like "large")
    ...
    Example:
        >>> model = build_cnn_encoder(CNNEncoderArch.large)
        >>> model = build_cnn_encoder("large", load_pretrained=True,
