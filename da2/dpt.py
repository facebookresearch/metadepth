...
HYDEN_DECODER_CONFIGS: Dict[str, List[int]] = {
    "large_htp": [256, 512, 1024, 1024],
}


# =============================================================================
# DINOv2 Encoder (OSS - Pure PyTorch)
# =============================================================================
...
def build_dinov2(
    ...
    Args:
        model_name: One of "vits", "vitb", "vitl", "vitg"
    ...
    model = DinoVisionTransformer(
        ...
        init_values=1.0,
        ffn_layer="mlp" if model_name != "vitg" else "swiglu",
...
# =============================================================================
# Full Depth Estimation Models (OSS)
# =============================================================================


class DepthAnythingFrozen(nn.Module):
    """
    Frozen DINOv2 + DPT head for inference only.

    Simplified version for inference without training features.
    """
    ...
    def __init__(
        ...
        self.depth_head.to(device).eval()

        # If a full checkpoint is provided (encoder + head), try to load decoder weights too
        if pretrained_weights_path:
            checkpoint = torch.load(
                pretrained_weights_path, map_location="cpu", weights_only=True
            )
            if isinstance(checkpoint, dict):
                # Extract potential depth_head.* prefix or direct keys
                ...
                if head_state:
                    missing, unexpected = self.depth_head.load_state_dict(
                        head_state, strict=False
                    )
                    if missing or unexpected:
                        logger.warning(
                            f"Depth head load from checkpoint had missing={len(missing)} unexpected={len(unexpected)}"
                        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Resize to 518x518 (DINOv2 standard size)
        x = F.interpolate(x, size=[518, 518], mode="bilinear", align_corners=True)
        ...
        out = F.interpolate(out, size=output_size, mode="nearest")
...
class DepthAnything(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            add_cnn_encoder: CNN encoder architecture for HyDen mode (e.g., "large_htp")
        """
        super().__init__()
